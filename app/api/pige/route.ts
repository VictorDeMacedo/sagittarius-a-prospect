...
export default function Dashboard(){
  ...
  const scrape = async () => {
    ...
    try{
      ...
      if(data?.error && data.error.includes('403')){
        setStatus('⚠️ LeBonCoin bloqué par DataDome (403) - mode démo actif')
      }else if(data?.error){
        setStatus(`⚠️ ${data.error.slice(0,150)} - mode démo`)
      }else{
  ...
  return (
    <div style={{padding:20, fontFamily:'system-ui', background:'#f8fafc', minHeight:'100vh'}}>
      ...
      <p>Orléans 30km | Fix ERR_NAME_NOT_RESOLVED via /api/pige | Supabase ✅ Fonction ✅</p>
      <div style={{display:'flex', gap:20, marginTop:20, flexWrap:'wrap'}}>
        <div style={{flex:1, minWidth:280, background:'white', padding:20, borderRadius:12}}>
          ...
          {status && <div style={{marginTop:12, background: status.includes('✅')?'#dcfce7':'#fef3c7', padding:10, borderRadius:8, fontSize:13, wordBreak:'break-word'}}>{status}</div>}
          <div style={{marginTop:12, fontSize:11, color:'#64748b'}}>Astuce: F12 > Console pour voir PIGE DATA</div>
        </div>
